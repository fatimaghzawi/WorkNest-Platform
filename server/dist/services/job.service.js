...
const listJobs = async (query = {}, { populate = true } = {}) => {
...
const assertJobOwner = (job, userId, userRole) => {
    if (userRole === 'admin')
        return;
...
const assertJobEditable = async (job, userRole) => {
...
const updateJob = async (id, userId, data, userRole = 'client') => {
...
const assertJobArchivable = async (job) => {
    ...
    if (job.status === 'in_progress') {
        throw new AppError('Cannot archive a job with an active project. Cancel the project first to refund escrow.', 400);
    }
    if (job.status === 'closed') {
    ...
    const project = await projectRepository.findByJobId(job._id.toString());
    if (project) {
        throw new AppError('Cannot archive a job that already has a project. Cancel the project first.', 400);
    }
};
const deleteJob = async (id, userId, userRole = 'client') => {
    const job = await jobRepository.findById(id);
    if (!job) {
        throw new AppError('Job not found', 404);
    }

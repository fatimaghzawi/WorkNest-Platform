...
const resolveTaskOrigin = (task) => {
    ...
    const role = task?.createdBy && typeof task.createdBy === 'object' ? task.createdBy.role : undefined;
...
const toClientTask = (task, attachmentCount = 0) => {
    ...
    const createdByObj = plain.createdBy && typeof plain.createdBy === 'object' ? plain.createdBy : null;
...
const verifyJobParticipant = async (job, jobId, userId, userRole) => {
    if (userRole === 'admin')
        return;
    if (userRole === 'client') {
...
const getWorkspaceAccess = async (jobId, userId, userRole) => {
    ...
    const canCollaborate = jobInProgress && (!project || projectStatus === 'active');
    const isWorkspaceRole = userRole === 'admin' || userRole === 'client' || userRole === 'freelancer';
    const permissions = {
        canCreate: Boolean(canCollaborate && isWorkspaceRole),
        canManageTasks: Boolean(canCollaborate && (userRole === 'freelancer' || userRole === 'admin')),
...
const assertWorkspaceWrite = async (jobId, userId, userRole) => {
...
const calculateJobProgress = async (jobId) => {
...
const syncProjectProgress = async (jobId) => {
    const project = await projectRepository.findByJobId(jobId);
    if (!project)
        return;
...
const listTasks = async (jobId, userId, userRole, query = {}) => {
    const { readOnly, permissions } = await getWorkspaceAccess(jobId, userId, userRole);
    const { page, limit, skip } = parsePagination(query);
    const origin = query.origin === 'client' || query.origin === 'freelancer' ? query.origin : undefined;
    const priority = query.priority === 'low' || query.priority === 'medium' || query.priority === 'high'
        ? query.priority
        : undefined;
    ...
    return {
        tasks: tasks.map((task) => toClientTask(task, countsByTask[task._id.toString()] || 0)),
        readOnly,
        permissions,
        progress,
...
const createTask = async (jobId, userId, userRole, data) => {
...
const hasContentEditFields = (data) => data.title !== undefined ||
    data.description !== undefined ||
    data.priority !== undefined ||
    data.dueDate !== undefined ||
    data.submissionNotes !== undefined;
const updateTask = async (jobId, taskId, userId, userRole, data) => {
    await assertWorkspaceWrite(jobId, userId, userRole);
    ...
    await syncProjectProgress(jobId);
    await notificationTriggers.workspaceTaskUpdated(jobId, userId, existing.title, updates.status || updated.status);
    return toClientTask(updated, await workspaceAttachmentRepository.countByTask(taskId));
};
const deleteTask = async (jobId, taskId, userId, userRole) => {
    await assertWorkspaceWrite(jobId, userId, userRole);
    const existing = await workspaceRepository.findById(taskId);
    if (!existing || getId(existing.jobId) !== jobId) {
        throw new AppError('Task not found', 404);
...
const seedDefaultTasksForJob = async (jobId, createdBy) => {
...
const getWorkspaceTeam = async (jobId, userId, userRole) => {
...
const toClientAttachment = (attachment) => {
    ...
    const uploader = plain.uploadedBy && typeof plain.uploadedBy === 'object' ? plain.uploadedBy : null;
    return {
        id: plain._id.toString(),
...
const assertWorkspaceAttachmentWrite = async (jobId, userId, userRole) => {
    await assertWorkspaceWrite(jobId, userId, userRole);
    if (userRole === 'client') {
        throw new AppError('Only freelancers can upload workspace attachments', 403);
    }
};
const listAttachments = async (jobId, userId, userRole, query = {}) => {
    await getWorkspaceAccess(jobId, userId, userRole);
    const { page, limit, skip } = parsePagination(query);
...
const listTaskDeliverables = async (jobId, userId, userRole, query = {}) => {
    await getWorkspaceAccess(jobId, userId, userRole);
    const { page, limit, skip } = parsePagination(query);
    const previewLimit = Math.min(typeof query.previewLimit === 'number'
        ? query.previewLimit
        : Number(query.previewLimit) || 6, 20);
...
const uploadAttachment = async (jobId, userId, userRole, file, data = {}) => {
    ...
    await notificationTriggers.workspaceAttachmentUploaded(jobId, userId, clientAttachment.fileName, clientAttachment.uploaderName);
    return clientAttachment;
};
const deleteAttachment = async (jobId, attachmentId, userId, userRole) => {
    await assertWorkspaceAttachmentWrite(jobId, userId, userRole);

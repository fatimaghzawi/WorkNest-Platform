...
const isPopulatedJob = (value) => Boolean(value && typeof value === 'object' && typeof value.title === 'string');
...
const assertInterviewAccess = (interview, userId, userRole) => {
...
const listMyInterviews = async (userId, userRole, query = {}) => {
...
const getInterview = async (id, userId, userRole) => {
...
const createInterview = async (actorId, actorRole, data) => {
...
const updateInterview = async (id, userId, userRole, data) => {
    const interview = await interviewRepository.findById(id);
    if (!interview) {
        throw new AppError('Interview not found', 404);
    }
...
const updateInterviewStatus = async (id, userId, userRole, status) => {
    const interview = await interviewRepository.findById(id);
    if (!interview) {
        throw new AppError('Interview not found', 404);
    }

...
const getClientJobIds = async (clientId) => Job.find({ clientId: toClientObjectId(clientId), deletedAt: null }).distinct('_id');
...
const monthlyJobActivity = async (monthsBack = 11) => {
    ...
    const proposalMap = new Map(proposalsByMonth.map((row) => [`${row._id.year}-${row._id.month}`, row.proposals]));
    const points = [];
...
const recentJobs = async (limit = 5) => {
    ...
    return jobs.map((job) => ({
        ...
        status: job.status,
        budget: job.budget,
        clientName: job.clientId && typeof job.clientId === 'object'
...
const clientJobPipelineStats = async (clientId) => {
    ...
    const budgetMap = new Map(rows.map((row) => [row._id, row.budget]));
...
const monthlyFreelancerActivity = async (freelancerId, monthsBack = 11) => {
    ...
    const acceptedMap = new Map(acceptedByMonth.map((row) => [`${row._id.year}-${row._id.month}`, row.accepted]));
    const points = [];
    const cursor = new Date(start);
    const now = new Date();
    while (cursor.getFullYear() < now.getFullYear() ||
...
const monthlyUserSignups = async (monthsBack = 11) => {
    ...
    const signupMap = new Map(signupsByMonth.map((row) => [`${row._id.year}-${row._id.month}`, row.signups]));
    const points = [];
    const cursor = new Date(start);
    const now = new Date();
    while (cursor.getFullYear() < now.getFullYear() ||

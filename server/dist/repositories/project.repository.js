...
const findByJobId = (jobId) => Project.findOne({ jobId });
const findByJobIdPopulated = (jobId) => Project.findOne({ jobId })
    .populate(CLIENT_POPULATE)
    .populate(FREELANCER_POPULATE)
    .populate(JOB_POPULATE);
const findAll = ({ filter, skip, limit, sort }) => Project.find(filter)
...
const updateById = (id, data) => Project.findByIdAndUpdate(id, data, { returnDocument: 'after', runValidators: true })
    .populate(CLIENT_POPULATE)
    .populate(FREELANCER_POPULATE)
    .populate(JOB_POPULATE);
const deleteById = (id) => Project.findByIdAndDelete(id);
const findOrCreateFromAcceptance = async ({ jobId, clientId, freelancerId, title, }) => {

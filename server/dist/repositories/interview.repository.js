...
const updateById = (id, data) => Interview.findByIdAndUpdate(id, data, { returnDocument: 'after', runValidators: true })
    .populate(CLIENT_POPULATE)
    .populate(FREELANCER_POPULATE)
    .populate(JOB_POPULATE);
const deleteById = (id) => Interview.findByIdAndDelete(id);
const deleteByJobId = (jobId) => Interview.deleteMany({ jobId });
const cancelActiveByJobId = (jobId) => Interview.updateMany({ jobId, status: { $in: ['scheduled', 'confirmed'] } }, { status: 'cancelled' });
module.exports = {
    create,
    findById,
    findAll,

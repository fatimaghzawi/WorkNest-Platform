...
const createTask = async (req, res) => {
    const task = await workspaceService.createTask(req.params.jobId, req.user.id, req.user.role, req.body);
    return sendSuccess(res, {
        statusCode: 201,
...
const updateTask = async (req, res) => {
    const task = await workspaceService.updateTask(req.params.jobId, req.params.taskId, req.user.id, req.user.role, req.body);
    return sendSuccess(res, {
...
const getTeam = async (req, res) => {
    const team = await workspaceService.getWorkspaceTeam(req.params.jobId, req.user.id, req.user.role);
    return sendSuccess(res, {
...
const listAttachments = async (req, res) => {
    const { attachments, meta } = await workspaceService.listAttachments(req.params.jobId, req.user.id, req.user.role, req.query);
    return sendSuccess(res, {
...
const deleteAttachment = async (req, res) => {
    await workspaceService.deleteAttachment(req.params.jobId, req.params.attachmentId, req.user.id, req.user.role);
    return sendSuccess(res, {

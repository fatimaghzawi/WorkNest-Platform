...
const getPaymentByProject = async (req, res) => {
    const payment = await paymentService.getPaymentByProject(req.params.projectId, req.user.id, req.user.role);
    return sendSuccess(res, {
...
const createCheckoutSession = async (req, res) => {
    const session = await paymentService.createCheckoutSession(req.params.projectId, req.user.id, req.body?.returnPath);
    return sendSuccess(res, {
...
const confirmCheckoutSession = async (req, res) => {
    const payment = await paymentService.confirmCheckoutSession(req.params.projectId, req.user.id, req.body?.sessionId);
    return sendSuccess(res, {

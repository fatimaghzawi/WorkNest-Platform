...
const feeFor = (amount: number, budget: number) => calculatePlatformFee(amount, budget);
...
async function createHireBundle(opts: {
  ...
  let projectStatus: 'active' | 'pending_review' | 'completed' | 'cancelled' = 'active';
  let progress = 0;
  ...
  const project = await Project.create({
    ...
    progress,
    githubLink:
      kind === 'completed' || kind === 'pending_review'
        ? 'https://github.com/worknest-demo/delivery'
        : undefined,
    deliveryNotes:
      kind === 'pending_review' || kind === 'completed'
...
async function seed() {
  ...
  if (!uri) throw new Error('MONGO_URI is missing from server/.env');
  ...
  const openJobs = await Job.insertMany([
    {
      ...
      description:
        'We need a React + TypeScript dashboard that visualizes MRR, churn, and cohort retention. Must integrate with our existing Nest API and support CSV export. Looking for clean charts, responsive layout, and sensible loading states.',
    ...
    },
    {
      ...
      description:
        'Improve conversion on our Shopify-like checkout. Produce Figma flows for cart → shipping → payment → confirmation with accessibility notes and a short prototype.',
    ...
    },
    {
      ...
      description:
        'Build a map-centric React UI for vehicle locations, route history, and driver status. WebSocket-friendly architecture preferred.',
      category: 'web-development',
      budget: 3200,
    ...
    },
    {
      ...
      description:
        'Create a client portal: login, project status, invoice list, and file downloads. Next.js + Tailwind preferred.',
      category: 'web-development',
      budget: 3500,
    ...
    },
    {
      ...
      description:
        'Cross-platform itinerary planner with offline drafts, shared trips, and map pins. Firebase backend is fine for MVP.',
  ...
  for (const [i, job] of openJobs.slice(0, 3).entries()) {
    const mine = await Proposal.create({
      jobId: job._id,
  ...
  const hireSpecs: Array<{
    ...
    kind: HireKind;
    competitors?: number[];
    interview?: 'scheduled' | 'completed' | 'confirmed';
  }> = [
    {
      clientIdx: 3,
      title: 'Donor reporting automation pipeline',
      description:
        'Automate weekly donor CSV transforms into stakeholder-ready dashboards. Python + SQL preferred with documented jobs and alerts.',
    ...
    },
    {
      ...
      description:
        'Stand up GitHub Actions, Docker images, staging/prod deploy pipelines, rollback docs, and basic monitoring hooks.',
      category: 'devops',
      budget: 2400,
    ...
    },
    {
      ...
      description:
        'Rebuild health landing sections in React with stronger accessibility and A/B-ready content blocks.',
      category: 'web-development',
      budget: 2200,
    ...
    },
    {
      ...
      description:
        'Prototype assignments and grading flows. Project later cancelled after funding change — partial work already shipped.',
      category: 'web-development',
      budget: 3000,
    ...
    },
    {
      ...
      description:
        'Admin console for sellers, orders, and disputes. Escrow not funded yet — waiting on deposit before workspace unlock.',
      category: 'web-development',
      budget: 5200,
    ...
    },
    {
      ...
      description:
        'Embeddable React widgets for CS health scores, renewal risk, and notes. Design tokens provided.',
      category: 'web-development',
      budget: 1900,
    ...
    },
    {
      ...
      description:
        'Audit and fix LCP/CLS on product listing and PDP. Ship measurable improvements with before/after notes.',
      category: 'web-development',
      budget: 1400,
  ...
  for (const spec of hireSpecs) {
    ...
    const jobStatus =
      spec.kind === 'completed' || spec.kind === 'cancelled_partial' ? 'closed' : 'in_progress';
    ...
    const winner = await Proposal.create({
      jobId: job._id,
      freelancerId: primary._id,
    ...
    for (const flIdx of spec.competitors || []) {
      ...
      if (String(freelancer._id) === String(primary._id)) continue;
      await Proposal.create({
        jobId: job._id,
  ...
  const rejectJob = await Job.create({
    clientId: C[2]._id,
  ...
  const rejectedMine = await Proposal.create({
    jobId: rejectJob._id,
    freelancerId: primary._id,
  ...
  // A couple of other freelancers' hired jobs so client/admin dashboards aren't empty of others
  {
    ...
    const otherProp = await Proposal.create({
      jobId: otherJob._id,
  ...
  const counts = {
    ...
    primaryProposals: await Proposal.countDocuments({ freelancerId: primary._id }),
    primaryProjects: await Project.countDocuments({ freelancerId: primary._id }),
  };

...
const isProductionSecret = (value) =>
  Boolean(value) && !value.startsWith('dev_') && !value.startsWith('your_');

const isEmailConfigured = (data) =>
  Boolean(data.SENDGRID_API_KEY) ||
...
const envSchema = z
  .object({
    ...
    MONGO_URI: z.preprocess(
      emptyToUndefined,
      z.string().min(1).default('mongodb://127.0.0.1:27017/worknest')
    ),

    JWT_SECRET: z.preprocess(
      emptyToUndefined,
      z.string().min(1).default('dev_jwt_secret_change_in_production')
    ),
    JWT_EXPIRES_IN: z.preprocess(emptyToUndefined, z.string().default('1d')),
    JWT_REFRESH_SECRET: z.preprocess(
      emptyToUndefined,
      z.string().min(1).default('dev_refresh_secret_change_in_production')
    ),
    JWT_REFRESH_EXPIRES_IN: z.preprocess(emptyToUndefined, z.string().default('30d')),
    JWT_COOKIE_EXPIRES_IN: z.preprocess(
      emptyToUndefined,
    ...
    EMAIL_FROM: z.preprocess(
      emptyToUndefined,
      z.string().default('WorkNest <no-reply@worknest.com>')
    ),
  ...
  .superRefine((data, ctx) => { //production only rules runs after the validation and add custom rules
    ...
    const productionRules = [
      ...
      },
      {
        ...
        message:
          'CLIENT_URL must be your deployed frontend URL (e.g. https://work-nest-platform-nu.vercel.app)',
      },
      {
        field: 'SENDGRID_API_KEY',
        valid: () => isEmailConfigured(data),
        message:
          'Set ELASTICEMAIL_API_KEY (recommended), SENDGRID_API_KEY, or EMAIL_USER + EMAIL_PASS for SMTP (local dev only)',
      },
      {
...
const parseEnv = () => { //parse the environment variables and validate them and fails fast if any error
...
const allowedOrigins = Array.from(
  new Set([...configuredClientOrigins, ...knownFrontendOrigins])
);

const env = {  //export the environment variables and make them available in the application

...
const errorHandler = (err, req, res, next) => {
  ...
  const message =
    env.isProduction && (isServerError || !err.isOperational)
  ...
  persistErrorLog(req, {
    ...
    statusCode: isServerError ? 500 : statusCode,
    category: 'system',
  ...
  return sendError(res, {
    statusCode: isServerError ? 500 : statusCode,
    message,
    errors: !env.isProduction && isServerError ? [{ field: 'stack', message: err.stack }] : [],
  });
};

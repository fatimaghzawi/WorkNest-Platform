...
const { withTimeout } = require('../../../common/utils/promise.util');
const { appPath, clientPath } = require('../../../common/utils/appUrls');
...
class EmailService {
  async verifyConnection() {
    if (!env.email.isConfigured) {
      const message =
        'Email is not configured. Set ELASTICEMAIL_API_KEY, SENDGRID_API_KEY, or EMAIL_USER + EMAIL_PASS.';
  ...
  private async sendViaElasticEmail({
    to,
    subject,
    html,
    text,

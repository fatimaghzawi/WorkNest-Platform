...
/** Atomic pending → held transition (idempotent under concurrent webhook + confirm). */
const markPendingDeposited = (projectId: string, data) =>
  Payment.findOneAndUpdate({ projectId, status: 'pending' }, data, {
    returnDocument: 'after',
    runValidators: true,
  }).lean();

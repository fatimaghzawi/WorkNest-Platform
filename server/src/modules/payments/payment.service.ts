...
const notificationTriggers = require('../notifications/notificationTriggers');
const { calculatePlatformFee } = require('./payments.constants');
const { clientPath } = require('../../common/utils/appUrls');
...
const toClientPayment = (doc, projectTitle?: string) => {
  if (!doc) return null;
...
const assertPaymentAccess = (payment, userId: string, userRole: string) => {
  const isClient = getId(payment.clientId) === userId;
  const isFreelancer = getId(payment.freelancerId) === userId;

  if (userRole === 'admin' || isClient || isFreelancer) return { isClient, isFreelancer };

  throw new AppError('You do not have access to this payment', 403);
};

const createPendingEscrow = async ({
  projectId,
  jobId,
  proposalId,
  clientId,
  ...
  if (existing) return toClientPayment(existing);
...
const markEscrowDeposited = async (
  projectId: string,
  cardMeta: {
    cardBrand?: string;
    cardLast4?: string;
  ...
  const now = new Date();
  const updated = await paymentRepository.markPendingDeposited(projectId, {
    status: 'held',
...
const extractCardMetaFromSession = async (session) => {
  ...
  if (!session.payment_intent || !stripe) return cardMeta;
...
const releaseEscrow = async (projectId: string) => {
  const payment = await paymentRepository.findByProjectId(projectId);
...
const refundEscrow = async (projectId: string) => {
  const payment = await paymentRepository.findByProjectId(projectId);
  if (!payment) return null;
...
const assertEscrowHeldForProject = async (projectId: string) => {
  const payment = await paymentRepository.findByProjectId(projectId);
  if (!payment) return;

  if (payment.status !== 'held') {
    throw new AppError(
      'Project escrow must be funded by the client before work can begin in the workspace',
      403
    );
  }
};

const getPaymentByProject = async (projectId: string, userId: string, userRole: string) => {
  const payment = await paymentRepository.findByProjectId(projectId);
  if (!payment) return null;
...
const getWalletSummary = async (userId: string, userRole: string) => {
  if (userRole === 'client') {
...
const listPayments = async (userId: string, userRole: string, query: Record<string, unknown> = {}) => {
  ...
  if (userRole === 'admin') {
    ...
    const projects = await Promise.all(
      projectIds.map((id) => projectRepository.findById(id).catch(() => null))
    );
    const titleMap = new Map(
      projects.filter(Boolean).map((project) => [getId(project._id), project.title])
    );
...
const mapPaymentsByProjectId = async (projectIds: string[]) => {

...
const { normalizeStoredFileUrl } = require('../../common/utils/upload.util');
...
const getPublicFreelancerProfile = async (freelancerId: string) => {
  ...
  const winRate =
    proposalsTotal > 0 ? Math.round((proposalsAccepted / proposalsTotal) * 100) : 0;
...
const assertCanViewClientProfile = async (clientId: string, requesterId: string, requesterRole: string) => {
...
const getPublicClientProfile = async (clientId: string, requesterId: string, requesterRole: string) => {

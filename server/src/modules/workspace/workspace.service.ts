...
const { assertStatusTransition } = require('./taskWorkflow');
...
const uploadService = require('../../shared/integrations/upload/upload.service');
...
const assertCanAccessJobFiles = async (jobId: string, userId: string, userRole: string) => {
  await getWorkspaceAccess(jobId, userId, userRole);
};

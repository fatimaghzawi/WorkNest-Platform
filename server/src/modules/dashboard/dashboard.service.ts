...
const getAnalyticsChart = async (query: Record<string, unknown> = {}) => {
...
const getLatestCustomers = async (query: Record<string, unknown> = {}) => {
  const limit = Math.min(Math.max(Number(query.limit) || 6, 1), 20);
  const users = await dashboardRepository.latestUsers(limit);
  return users;
};

const getRecentJobs = async (query: Record<string, unknown> = {}) => {
...
const getStatistics = async (query: Record<string, unknown> = {}) => {
  const months = Math.min(Math.max(Number(query.months) || 12, 3), 24);
  ...
  const [
    ...
    dashboardRepository.countByStatus(require('../jobs/job.model').default, [
      'open',
      'in_progress',
      'closed',
    ]),
    dashboardRepository.countByStatus(require('../proposals/proposal.model').default, [
      'pending',
      'accepted',
      'rejected',
    ]),
    dashboardRepository.countByStatus(require('../projects/project.model').default, [
      'active',
      'completed',
      'cancelled',
    ]),
    dashboardRepository.countByStatus(require('../interviews/interview.model').default, [
      'scheduled',
      'confirmed',
      'completed',
      'cancelled',
...
const getFreelancerDashboard = async (freelancerId: string) => {
  const now = new Date();
  const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
  const startOfPrevMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
  const endOfPrevMonth = new Date(now.getFullYear(), now.getMonth(), 0, 23, 59, 59, 999);

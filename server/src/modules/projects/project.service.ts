...
const jobCleanupService = require('../jobs/jobCleanup.service');
...
const assertProjectAccess = (project, userId: string, userRole: string) => {
...
const ensureJobClosedForCompletedProject = async (clientProject) => {
  if (clientProject.status !== 'completed') return clientProject;

  const jobId = clientProject.jobId;
  if (!jobId) return clientProject;
...
const attachTaskProgress = async (clientProjects) => {
  ...
  if (jobIds.length === 0) return clientProjects;
...
const listProjects = async (userId: string, userRole: string, query: Record<string, unknown> = {}) => {
  ...
  else if (userRole === 'freelancer') filter.freelancerId = userId;
  ...
  const paymentMap = await paymentService.mapPaymentsByProjectId(
    projects.map((project) => project._id.toString())
  );

  const withPayments = projects.map((project) =>
    toClientProject(project, paymentMap.get(project._id.toString()) || null)
  );
...
const getProject = async (id: string, userId: string, userRole: string) => {
  const project = await projectRepository.findById(id);
  if (!project) throw new AppError('Project not found', 404);
...
const getProjectStats = async (userRole: string) => {
  if (userRole !== 'admin') {
...
const acceptProject = async (id: string, userId: string, userRole: string) => {
  const project = await getProjectById(id);

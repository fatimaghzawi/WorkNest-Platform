...
const { generateSecureToken, hashToken } = require('../../common/utils/token.util');
const { signAccessToken } = require('../../common/utils/jwt');
const emailService = require('../../shared/integrations/email/email.service');
...
const { logWarning, logInfo } = require('../../common/utils/logger');
...
const getGithubAuthorizationUrl = (role?: string) => {
...
const completeGithubOAuth = async (code?: string, state?: string) => {
  if (!githubConfig.isConfigured) {
    throw new AppError('GitHub sign-in is not configured', 503);
  }
...
const refreshSession = async (rawRefreshToken: string) => {
  if (!rawRefreshToken) {
    throw new AppError('Refresh token required', 401);
  }

  const tokenHash = hashToken(rawRefreshToken);
  const stored = await refreshTokenRepository.consumeByTokenHash(tokenHash);

  if (!stored) {
    // Possible reuse after rotation — revoke all sessions for safety if hash existed recently.
  ...
  if (!user || !user.isActive) {
    await refreshTokenRepository.deleteAllForUser(stored.userId.toString());

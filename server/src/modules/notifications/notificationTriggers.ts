...
const eventEmailService = require('../../shared/integrations/email/eventEmail.service');
...
const notifyOtherProposalsRejected = async (jobId: string, exceptProposalId: string, jobTitle: string) => {
...
const projectRevisionRequested = async (project, reviewNotes?: string) => {
  const jobTitle = project.jobId?.title || project.title || 'the project';
...
const projectCancelled = async (project, reason?: string) => {
  const jobTitle = project.jobId?.title || project.title || 'the project';
...
const getWorkspaceParties = async (jobId: string) => {
...
const notifyWorkspaceOtherParty = async (
  jobId: string,
  actorId: string,
  ...
  const recipientId =
    parties.clientId === actorId ? parties.freelancerId : parties.clientId;
...
const workspaceTaskCreated = async (jobId: string, actorId: string, taskTitle: string) => {
...
const workspaceTaskUpdated = async (
  jobId: string,
  actorId: string,
  taskTitle: string,
  status?: string
...
const workspaceAttachmentUploaded = async (
  jobId: string,
  actorId: string,
  fileName: string,
  uploaderName: string
  ...
  const parties = await getWorkspaceParties(jobId);
  if (!parties.clientId || parties.clientId === actorId) return;
...
const interviewUpdated = async (interview, actorId: string) => {
  const recipientId =
    getId(interview.clientId) === actorId
...
const interviewStatusChanged = async (interview, status: string, actorId: string) => {
  const recipientId =
    getId(interview.clientId) === actorId
      ? getId(interview.freelancerId)
      : getId(interview.clientId);

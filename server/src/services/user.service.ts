...
const assertNotSelf = (targetUserId: string, currentUserId: string, action: string) => {
...
const updateUser = async (id: string, data, currentUserId: string) => {
  const user = await userRepository.findById(id);
...
const deleteUser = async (id: string, currentUserId: string) => {
  ...
  if (activeJobs > 0) {
    throw new AppError(
      'Cannot deactivate a user with open or in-progress jobs. Close or cancel them first.',
      400
    );
  }
  ...
  if (activeProjects > 0) {
    throw new AppError(
      'Cannot deactivate a user with active projects. Complete or cancel them first.',
      400
    );
  }
  ...
  if (heldPayments > 0) {
    throw new AppError(
      'Cannot deactivate a user while escrow funds are still held on active payments.',
      400
    );
  }

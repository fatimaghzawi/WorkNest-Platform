...
const FREELANCER_TRANSITIONS: Record<string, string[]> = {
  todo: ['in_progress'],
  in_progress: ['review'],
  review: ['in_progress'],
  done: [],
...
const assertStatusTransition = (
  ...
  if (userRole === 'freelancer') {
    ...
    throw new AppError(
      `Freelancers can move tasks from ${formatStatus(fromStatus)} to ${allowed.map(formatStatus).join(' or ') || 'nowhere'}`,
      400
    );
  }
...
const canTransition = (userRole: string, fromStatus: string, toStatus: string) => {
...
const getAllowedTargetColumns = (userRole: string, fromStatus: string) => {
  if (userRole === 'admin') return ['todo', 'in_progress', 'review', 'done'];
  if (userRole === 'freelancer') return FREELANCER_TRANSITIONS[fromStatus] || [];
  if (userRole === 'client') return CLIENT_TRANSITIONS[fromStatus] || [];
  return [];
};

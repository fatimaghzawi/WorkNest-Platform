...
export function StoryRibbon({ chapter }: { chapter?: Beat['chapter'] }) {
  ...
  if (!chapter || chapter === 'GAP' || chapter === 'ARC') {
    return (
      <div className="pointer-events-none absolute left-1/2 top-3 z-50 hidden -translate-x-1/2 md:block">
  ...
  return (
    <div className="pointer-events-none absolute left-1/2 top-3 z-50 hidden -translate-x-1/2 md:block">
      <div className="flex items-center gap-1 rounded-full bg-white px-2 py-1.5 shadow-card ring-1 ring-wn-line">
...
export function CastChip({ perspective }: { perspective: Beat['perspective'] }) {
  ...
  return (
    <div className="pointer-events-none absolute right-5 top-14 z-50 hidden md:block">
